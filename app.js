...
//     };
// }
// Safely getting references to HTML elements
...
function generateResume() {
    ...
    if (!resumeContentDiv)
        return; // Ensure resumeContentDiv exists before proceeding
...
function downloadResumeAsPDF() {
    ...
    if (resumeForm) {
        resumeForm.addEventListener("submit", function (event) {
            ...
            generateResume(); // Generate the resume content
            if (resumeForm)
    ...
    if (editButton) {
        editButton.addEventListener("click", function () {
            if (resumeForm)
                resumeForm.style.display = "block"; // Show the form for editing
            if (resumeOutput)
                resumeOutput.style.display = "none"; // Hide the resume output
            if (resumeForm)
                resumeForm.scrollIntoView({ behavior: "smooth" }); // Scroll to form
        });
    }
